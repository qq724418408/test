...
import com.boc.jx.baseUtil.util.DoubleKeyValueMap;
...
public class EventListenerManager {
	...
	public static void addEventMethod(ViewFinder finder, ViewInjectInfo info,
		...
		try {
			...
			if (view != null) {
				...
				if (!addNewMethod) {
					dynamicHandler = new DynamicHandler(handler, view);
					dynamicHandler.addMethod(methodName, method);
				...
				Method setEventListenerMethod = view.getClass().getMethod(listenerSetter,
						listenerType);
	...
	public static class DynamicHandler implements InvocationHandler {
		...
		private WeakReference<View> handlerView;

		public DynamicHandler(Object handler, View view) {
			this.handlerRef = new WeakReference<Object>(handler);
			handlerView = new WeakReference<View>(view);
		}
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			if (handler != null) {
				...
				if (method != null) {
                    ...
                    if (checkNet!=null&&!NetworkUtils.isNetworkAvailable(handlerView.get().getContext())) {
						ToastUtils.show(handlerView.get().getContext(), "当前无网络", 0);
                    	return null;
					}
                    // 重复点击注解
					...
					if (duplicate!=null) {
						// 重复点击加上了注解标签
						if (Utils.isFirstClick(duplicate.value())) {
							return method.invoke(handler, args);
						} else {
							LogUtils.e("tag = "+duplicate.value());
							return null;
						}
					}
					if (Utils.isFirstClick(handlerView.get().getId())) {
						return method.invoke(handler, args);
					} else {
