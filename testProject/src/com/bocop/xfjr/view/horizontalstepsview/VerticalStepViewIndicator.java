 ...
 * 描述：
 */
public class VerticalStepViewIndicator extends View implements Runnable {
    ...
    private void init()
    {
        ...
        mUnCompletedPaint.setStrokeWidth(8);
...
//        mUnCompletedPaint.setPathEffect(mEffects);
        ...
        mCompleteIcon =  getContext().getResources().getDrawable( R.drawable.xfjr_complted);//已经完成的icon
        mAttentionIcon =  getContext().getResources().getDrawable(R.drawable.xfjr_attention);//正在进行的icon
        mDefaultIcon =  getContext().getResources().getDrawable(R.drawable.xfjr_default_icon);//未完成的icon
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        if(mStepNum > 0)
        {
            ...
            mHeight = (int) (getPaddingTop() + getPaddingBottom() + mCircleRadius * 2 * mStepNum + (mStepNum ) * mLinePadding);
        }
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        for(int i = 0; i < mCircleCenterPointPositionList.size(); i++)
        {
            ...
            } else if(i == mComplectingPosition && mCircleCenterPointPositionList.size() != 1)
            {
                ...
                canvas.drawCircle(mCenterX, currentComplectedXPosition, mAttentionIcon.getIntrinsicHeight()*0.5f*1.1f, mCompletedPaint);//mCircleRadius * 1.1f
...
//        this.mStepBeanList = stepsBeanList;
        ...
        if(stepsBeanList!=null&&stepsBeanList.size()>0)
        {
            for(int i = 0;i<mStepNum;i++)
            {
                StepBean stepsBean = stepsBeanList.get(i);
                {
    ...
    public void run() {
        mOnDrawListener.ondrawIndicator();
        Log.e(TAG_NAME,"mOnDrawListener");
    }
